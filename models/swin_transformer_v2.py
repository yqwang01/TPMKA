...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, mode, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0], 
                 num_var=7, use_var=False, **kwargs):
        super().__init__()
        ...
        if self.use_var:
            ...
            self.concat_linear = nn.Linear(self.num_features + 64, self.num_features)
        self.hidden_representation = nn.Linear(self.num_features, 512)
...
def load_pretrained(config, model, pretrained_path):
...
def swin_transformer_v2_base(pretrained, **kwargs):
    
    model = SwinTransformerV2(img_size=256, drop_path_rate=0.5, embed_dim=128, 
                              depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],
                              window_size=8, **kwargs)

    if pretrained:
        swin_transformer_path = './models/swinv2_base_patch4_window8_256.pth'
        load_pretrained(kwargs, model, swin_transformer_path)
    return model


def swin_transformer_v2_small(pretrained, **kwargs):
    
    model = SwinTransformerV2(img_size=256, drop_path_rate=0.3, embed_dim=96, 
                              depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24],
                              window_size=8, **kwargs)

    if pretrained:
        swin_transformer_path = './models/swinv2_small_patch4_window8_256.pth'
        load_pretrained(kwargs, model, swin_transformer_path)
    return model


def swin_transformer_v2_tiny(pretrained, **kwargs):
    
    model = SwinTransformerV2(img_size=256, drop_path_rate=0.2, embed_dim=96, 
                              depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                              window_size=8, **kwargs)

    if pretrained:
        swin_transformer_path = './models/swinv2_tiny_patch4_window8_256.pth'
        load_pretrained(kwargs, model, swin_transformer_path)
    return model
